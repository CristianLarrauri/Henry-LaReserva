...
export default function PlayerInscription() {
	...
	const handlePopUp1 = (e) => {
		...
		if (Object.values(errors1).length > 0) {
			setPopUpError({
				title: 'Error!',
				msg: 'Por favor, revisá los datos ingresados del jugador'
			});
		} else if (
			player1.surname.length == 0 ||
	...
	const handlePopUp3 = (e) => {
		...
		if (Object.values(errors3).length > 0) {
			...
			player3.surname.length == 0 ||
			player3.name.length == 0 ||
			player3.dni == 0
		) {
			setPopUpError({ title: 'Error!', msg: 'Por favor, completá los campos' });
		} else {
	...
	const handlePopUp4 = (e) => {
		...
		if (Object.values(errors4).length > 0) {
			...
			player4.surname.length == 0 ||
			player4.name.length == 0 ||
			player4.dni == 0
		) {
			setPopUpError({ title: 'Error!', msg: 'Por favor, completá los campos' });
		} else {
	...
	const handlePopUp5 = (e) => {
		...
		if (Object.values(errors5).length > 0) {
			...
			player5.surname.length == 0 ||
			player5.name.length == 0 ||
			player5.dni == 0
		) {
			setPopUpError({ title: 'Error!', msg: 'Por favor, completá los campos' });
		} else {
	...
	const handlePopUp6 = (e) => {
		...
		if (Object.values(errors6).length > 0) {
			...
			player6.surname.length == 0 ||
			player6.name.length == 0 ||
			player6.dni == 0
		) {
			setPopUpError({ title: 'Error!', msg: 'Por favor, completá los campos' });
		} else {
	...
	const handlePopUp7 = (e) => {
		...
		if (Object.values(errors7).length > 0) {
			...
			player7.surname.length == 0 ||
			player7.name.length == 0 ||
			player7.dni == 0
		) {
			setPopUpError({ title: 'Error!', msg: 'Por favor, completá los campos' });
		} else {
	...
	const handlePopUp8 = (e) => {
		...
		if (Object.values(errors8).length > 0) {
			...
			player8.surname.length == 0 ||
			player8.name.length == 0 ||
			player8.dni == 0
		) {
			setPopUpError({ title: 'Error!', msg: 'Por favor, completá los campos' });
		} else {
	...
	const handlePopUp9 = (e) => {
		...
		if (Object.values(errors9).length > 0) {
			...
			player9.surname.length == 0 ||
			player9.name.length == 0 ||
			player9.dni == 0
		) {
			setPopUpError({ title: 'Error!', msg: 'Por favor, completá los campos' });
		} else {
	...
	const handlePopUp10 = (e) => {
		...
		if (Object.values(errors10).length > 0) {
			...
			player10.surname.length == 0 ||
			player10.name.length == 0 ||
			player10.dni == 0
		) {
			setPopUpError({ title: 'Error!', msg: 'Por favor, completá los campos' });
		} else {
	...
	const handleSubmit = (e) => {
		...
		if (!teamName) {
			setPopUpError({
				title: 'Error!',
				msg: 'Colocale un nombre a tu equipo!'
			});
		} else if (
			Object.values(errors1).length > 0 ||
			Object.values(errors2).length > 0 ||
			Object.values(errors3).length > 0 ||
			Object.values(errors4).length > 0 ||
			Object.values(errors5).length > 0 ||
			Object.values(errors6).length > 0 ||
			Object.values(errors7).length > 0 ||
			Object.values(errors8).length > 0
		) {
			setPopUpError({
				title: 'Error!',
				msg: 'Faltan datos o hay datos incorrectos.'
			});
			...
			setPopUpError({
				title: 'Error!',
				msg: 'Por favor, lee atentamente la condicion final y tilda la casilla "Entiendo".'
			});
	...
	const handleErrorSelect = (e) => {
		if (e.target.value === 'undefined') {
			setErrorSelect({
				...errorSelect,
				errorSelect: 'Elige un torneo para la inscripción'
			});
		} else {
			setErrorSelect({});
		}
	...
	return (
		<div className="w-full min-h-screen flex flex-col justify-between">
			...
			<div className="min-h-screen flex flex-col items-center relative">
				<Link to="/home">
					<button
						className="absolute w-[200px] h-[70px] bg-green-500
				...
				<h2 className="text-3xl font-bold text-green-500 mb-10 mt-24 animate-appear">
					Inscribi a tu equipo
				</h2>
				...
				<form className="bg-gray-100 w-5/6 flex flex-col items-center text-center min-w-[350px] shadow shadow-gray-700 animate-appear">
					...
					<div className="bg-gray-200 w-5/6 min-w-[330px] my-6 p-3 shadow shadow-gray-700">
						<b> Por favor, completá con los datos REALES de los jugadores.</b>{' '}
						<br /> Recordá que los datos serán pedidos por el personal de la
						cancha antes de iniciar el partido.
						<br />
						<br />
						<b> A TENER EN CUENTA: </b>
						<ul>
							<li>
								En caso de ser extranjero y no contar con un dni argentino:
								Completa la casilla de dni del jugador con los primeros 8
								digitos de su identificación en caso de que posea más o en caso
								contrario completalo con números CERO al final
							</li>
						</ul>
					</div>

					<div className="bg-gray-200 w-5/6 min-w-[330px] my-6 p-3 shadow shadow-gray-700">
						<h2 className="text-2xl text-green-500 font-medium mt-4 text-center">
							Jugador 1
						</h2>
						...
						>
							...
							<div
								className="absolute bg-red-500 p-2 text-white font-medium
					...
					{confirm8 ? (
						<div className="bg-gray-200 w-5/6 min-w-[330px] my-6 p-3 shadow shadow-gray-700 flex items-center flex-col relative">
							<h3>
								<b>CONDICIÓN NECESARIA</b>
							</h3>

							<p className="mt-3">
								Todos los datos suministrados deben ser <b>VERIDICOS</b>. En
								caso de que los datos no sean comprobables o incorrectos al
								momento de arrancar el partido, el equipo quedará{' '}
								<b>DESCALIFICADO.</b>
							</p>
							<div className="flex mt-6">
								...
								/>
								<p className="ml-1 font-medium text-lg">
