...
export default function DashBoardAdmin() {
	...
	const handleUnban = (e) => {
		...
		dispatch(banUser(e.target.value));
		setPopUpError({title: 'Exito', msg: 'Se ha quitado el baneo al usuario.'});
		axios.post("http://localhost:3001/email",payload)
			.then((data)=>{
				return data
			})
	...
	const handleDeleteReview = (e) => {
		...
		setPopUpError({title: 'Exito', msg: 'El comentario ha sido definitivamente eliminado.'});
	};
	...
	return (
		<div className="min-h-screen flex flex-col justify-between items-center bg-gray-100">
			...
			<Nav />
			<h1 className="text-4xl font-bold pt-6 text-gray-700">Panel de administrador</h1>
			...
			<div className="w-full text-gray-700 flex flex-col items-center pt-6 flex-wrap animate-appear">
				...
				<div className="bg-white w-6/12 text-center p-3 min-w-fit h-full shadow shadow-gray-700 m-3">
					...
					<h2 className='text-xl font-medium mb-3'>Administracion de usuarios</h2>
					
					<table className='border border-black w-full '>
                        {allUsers?.map((e) => (
                        <tr key={e.id} value={e.id} className='border-y border-black'>
                            <td className='mx-3 text-lg font-medium break-words max-w-[300px]'>{e.name}</td>
				...
				<div className="bg-white w-6/12 text-center p-3 min-w-fit h-full shadow shadow-gray-700 m-3">
					<h2 className='text-xl font-medium mb-3'>Administracion de torneos</h2>
					...
					<table className='border border-black w-full'>
						...
						{allTournaments?.map((e) => (
							<tr key={e.name} value={e.id} className='border-y border-black'>
								...
								<td className={`mx-3 text-lg ${e.enabled?"text-green-700":"text-red-700"} font-medium break-words max-w-[300px]`}>{e.name}</td>
								...
								<td>
									<Link to={`/admin/modify/${e.id}`}>
										<button className='mx-3 cursor-pointer hover:text-green-700 duration-300 font-medium text-lg'>Modifica el torneo</button>
									</Link>
								</td>
				...
				<div className="bg-white w-6/12 text-center p-3 h-full shadow shadow-gray-700 m-3 min-w-fit">
					<h2 className="text-xl font-medium mb-3">Administrar reseñas reportadas</h2>

					<table className='border border-black w-full'>
						{adminReviews.length?
							adminReviews.map((e) => (
							<tr key={e.id} value={e.id} className='border-y border-black flex flex-wrap items-center justify-around'>
								<td className='text-lg px-3 font-medium break-words max-w-[300px]'>{e.nombreUsuario}</td>
								<td className='text-lg px-3 text-left w-[300px] break-words'>{e.comentario}</td>
						...
						<h2 className='text-xl font-medium text-gray-700 p-2'>No hay ninguna reseña que requiera atencion por ahora.</h2>
						}
					</table>
				</div>
