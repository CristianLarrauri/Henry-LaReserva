		...
		<div className='w-full min-h-screen flex flex-col justify-between bg-gray-200'>
			...
			<div className='flex justify-start flex-col'>{/*div principal*/}
				...
				<div className='w-full flex flex-wrap justify-center md:flex-nowrap md:justify-start'>{/*div superior*/}
					...
					flex-col lg:flex-row lg:items-end relative overflow-hidden min-h-[532px]'>{/*green card*/}
						...
						<div className=' flex flex-col xl:flex-row z-10'>
							...
							<div className='w-6/6 flex flex-col justify-around mx-3'>{/*Title container*/}
								<h2 className='text-white text-4xl font-bold'>{nextTournaments.next[0]?
								nextTournaments.next[0].name:"Torneo la reserva"}</h2>
								
								<div className='flex justify-around text-white flex-wrap'>

									<div className='flex items-center m-3'>
										<BsCalendarDate className='text-3xl'/>
										<p className='ml-2 text-xl'>{nextTournaments.next[0]?
										nextTournaments.next[0].dateInit:"00:00:00 AM"}</p>
									</div>

									<div className='flex items-center m-3 text-xl'>
										<BiCategory className='text-3xl'/>
										<p>{nextTournaments.next[0]?
										nextTournaments.next[0].category:"Cargando.."}</p>
									</div>

									<div className='flex items-center m-3 text-xl'>
										<BsFillPersonFill className='text-3xl'/>
										<p>{nextTournaments.next[0]?
										nextTournaments.next[0].genre:"Cargando.."}</p>
									</div>
						...
						<div className='flex flex-col-reverse items-end mx-3 h-full'>
							<Link to={`/inscription?id=${
								nextTournaments.next[0]?nextTournaments.next[0].id:'0'}`} className='bg-white min-w-[180px] h-[80px] rounded-full m-8 z-50
